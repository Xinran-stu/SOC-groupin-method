...
!!!!!!!! IMPORTANT:              !!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!! This file is a modification of wntr package. To use this toolkit, you must install wntr first.
...
class ENepanet():
    ...
    def __init__(self, inpfile='', rptfile='', binfile=''):
        ...
        #             raise E1
        #         pass
        self.ENlib = ctypes.windll.LoadLibrary('E:\SOC\epanet2.dll')
        return
    ...
    def ENsetnodevalue(self,iIndex, iCode, setValue):
        ...
        iCode : int
            Node parameter code (see toolkit.optNodeParams)
        setValue: the value to be set

        """
        self.errcode = self.ENlib.ENsetnodevalue(iIndex, iCode, ctypes.c_float(setValue))
        self._error()

        return

    def ENsetlinkvalue(self,iIndex, iCode, setValue):
        """set parameter value for a link
        ...
        """
        self.errcode = self.ENlib.ENsetlinkvalue(iIndex, iCode, ctypes.c_float(setValue))
        self._error()
        
        return

    def ENgetmultiparams(self, objectName, iIndexs, iCode):
        """get a series of parameter values

        Parameters
        -------------
        objectName: string(Node or Link)
        iIndex : list
            Link indexs
        iCode : int
            Link parameter code (see toolkit.optLinkParams)
        ...
        params = []
        if objectName == 'Node':
            for i in range(len(iIndexs)):
                params.append(self.ENgetnodevalue(iIndexs[i],iCode))
        elif objectName == 'Link':
            for i in range(len(iIndexs)):
                params.append(self.ENgetlinkvalue(iIndexs[i],iCode))
        else:
            raise EpanetException('objectName should be "node" or "link"')
        return params
